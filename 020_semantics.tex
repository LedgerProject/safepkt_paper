...
\begin{definition}[Semantics of terms]\label{def:semantics_terms}
  ...
  \begin{align*}
    ...
    \end{cases}\\
    &\text{where $\mathsf{e}\neq \mathtt{mut}\borrow\mathsf{w}$,
    $\den[\mathsf{e}]{s}=\langle v,h'\rangle$ and $\ell$ is fresh}\\
    \denl[\mathtt{let\ mut}\ x=\mathtt{mut}\borrow\mathsf{w}]{m}{s}&=
    \begin{cases}
      \mathit{undefined} & \text{if $\ell_{\mathsf{w}} \in \ran(h)$}\\
      \langle\rho[x\to^m\ell],h[\ell\to \ell_{\mathsf{w}}]\rangle & \text{otherwise}
    \end{cases}\\
    &\text{where $\ell_{\mathsf{w}}=\loc(\mathsf{w},s)$ and $\ell$ is fresh}\\
