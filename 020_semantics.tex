...
\begin{example}
  ...
  \begin{itemize}
    ...
    %
    \item Let $s_2 = \rho_2 \star h_2$.
    Then, we have $\den[\mathtt{mut}\borrow x]{s_2} =
    \langle\loc(x,s_2),h_2\rangle = \langle\ell_x, h_2\rangle$.
    Therefore,
    $\denl[\mathtt{let\ mut}\ z = \mathtt{mut}\borrow x]{m}{s_2} =
    \langle \rho_3,\ h_3\rangle$
    where
    \begin{align*}
      \rho_3 & = \rho_2[z\to^m\ell_z] =
      \{z\to^m\ell_z,\ y\to^m\ell_y,\ x\to^m\ell_x\}\\
      h_3 & = h_1[\ell_y\to \ell_x] =
  ...
  \[
    ...
    \}^m
  \]
  Then, starting from $s_0$ as above, we get the same store $\rho_3\star h_3$
  at the end.
\end{example}
