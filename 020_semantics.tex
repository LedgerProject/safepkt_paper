...
\begin{definition}[Semantics of assignment]\label{def:semantics_assigmment}
  ...
  \begin{align*}
    \denl[\mathsf{w}=\mathsf{e}]{l}{s}&=\langle\rho,h''[\ell\to v]\rangle & \text{if the static type of $\mathsf{e}$ has copy semantics}\\
    \denl[\mathsf{w}=\mathsf{e}]{l}{s}&=\langle\rho,h''[\ell\Rightarrow v]\rangle & \text{if the static type of $\mathsf{e}$ has copy semantics}
  \end{align*}
  where $\den[e]{s}=\langle v,h'\rangle$, $\ell=\loc(\mathsf{w},\langle\rho,h'\rangle)$ and
  $h''=\drop(\{\ell\},h')$.
\end{definition}
