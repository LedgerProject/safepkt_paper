...
\begin{definition}[Semantics of leftvalues]\label{def:semantics_leftvalues}
  Given $s=\rho\star h$ and $\mathsf{w}\in\Leftvalues$, let $\ell=\loc(\mathsf{w},s)$.
  We define
  \[
...
\begin{definition}[Semantics of terms]\label{def:semantics_terms}
  ...
  \begin{align*}
    ...
    \begin{cases}
      ...
      \langle\rho,h''[\ell\Rightarrow v]\rangle & \text{otherwise, if $a$ is $\Rightarrow$}\\
      \langle\rho,h''[\ell\leadsto v]\rangle & \text{otherwise, if $a$ is $\leadsto$ and $v$ is not borrowed in $h'$}\\
      \langle\rho,h''[\ell\to v]\rangle & \text{otherwise, if $a$ is $\to$ and $v$ is not borrowed as mutable in $h'$}\\
      \text{undefined} & \text{otherwise,}
    \end{cases}\\
    &\text{where $\den[\mathsf{e}]{s}=\langle v,h',a\rangle$, $\ell=\loc(x,\langle\rho,h'\rangle)$ and
      $h''=\drop(\{\ell\},h')$}\\
    \mbox{}\\
    \denl[*\mathsf{w}=\mathsf{e}]{m}{s}&=
    \begin{cases}
      \text{undefined} & \text{if $\ell$ is borrowed in $h'$ and $\ell_\mathsf{w}\leadsto\ell\not\in h$}\\
      \langle\rho,h''[\ell\Rightarrow v]\rangle & \text{otherwise, if $a$ is $\Rightarrow$}\\
      \langle\rho,h''[\ell\leadsto v]\rangle & \text{otherwise, if $a$ is $\leadsto$ and $v$ is not borrowed in $h'$}\\
      \langle\rho,h''[\ell\to v]\rangle & \text{otherwise, if $a$ is $\to$ and $v$ is not borrowed as mutable in $h'$}\\
      \text{undefined} & \text{otherwise,}
    \end{cases}\\
    &\text{where $\den[\mathsf{e}]{s}=\langle v,h',a\rangle$, $\loc(\mathsf{w})=\ell_\mathsf{w}$, $h(\ell_\mathsf{w})=\ell$ and
      $h''=\drop(\{\ell\},h')$}\\
    \mbox{}\\
    \denl[\mathtt{let\ mut}\ x=\mathsf{e}]{m}{s}&=
    \begin{cases}
      \langle\rho[x\to^m\ell],h'[\ell\Rightarrow v]\rangle & \text{if $a$ is $\Rightarrow$}\\
      \langle\rho[x\to^m\ell],h'[\ell\leadsto v]\rangle & \text{otherwise, if $a$ is $\leadsto$ and $v$ is not borrowed in $h'$}\\
      \langle\rho[x\to^m\ell],h'[\ell\to v]\rangle & \text{otherwise, if $a$ is $\to$ and $v$ is not borrowed as mutable in $h'$}\\
      \text{undefined} & \text{otherwise,}
    \end{cases}\\
    &\text{where $\den[\mathsf{e}]{s}=\langle v,h',a\rangle$ and $\ell$ is fresh}\\
    \mbox{}\\
