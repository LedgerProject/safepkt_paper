...
\begin{definition}[Abstract stores]
  ...
  \begin{itemize}
  \item exactly one binding exists in $a$ for each given variable in $\dom(\kappa)$ (exhaustivity);
  \item for each binding $x\to^m t\in a$ if in $t$ occurs
    $x\to^m\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$
    or $x\to^m\mutborrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$
    then there exists $t'$ such that
    for all $1\le i\le n$ we have $\locreada(\mathsf{w}_i,a)=\langle t',\_\rangle$ (type consistency)
  \end{itemize}
  where, for $\mathsf{w}\in\Leftvalues$, the partial function
  ...
  \begin{align*}
    ...
    \langle\locreada(\mathsf{w}_1,a),\mathit{true}\rangle & \text{if $\locreada(\mathsf{w},a)=\langle\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\},b\rangle$}\\
    \langle\locreada(\mathsf{w}_1,a),\mathit{true}\rangle & \text{if $\locreada(\mathsf{w},a)=\langle\mutborrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\},b\rangle$}\\
