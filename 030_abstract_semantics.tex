...
\begin{definition}
  ...
  $a\sqcap\top=a$, $\top\sqcap a=a$, $\bot\sqcap a=\bot$, $a\sqcap a=\bot$
  and, when $a_1,a_2\not=\top,\bot$,
  \[
...
\begin{proof}
  ...
  \begin{itemize}
    \item (Reflexivity) Let $a\in\mathbb{AS}_\kappa$. If $a=\top$ ot $a=\bot$, then
    $a\sqsubseteq a$.
    ...
    we necessarily have $a_1=\top$, so $a_1=a_2$.
    If $a_2=\bot$ then, as $a_1\sqsubseteq a_2$, we necessarily have $a_1=\bot$, so $a_1=a_2$.
    Otherwise, we necessarily
    have $a_1\neq\top,\bot$. Moreover, as $a_1 \sqsubseteq a_2$, for every
  ...
  \begin{itemize}
    ...
    Let $k\in I$. If $a_k=\top$ then we have
    $\mu_I \sqsubseteq a_k$. If $a_k=\bot$ then
    $\mu_I=\bot$ and $\mu_I\sqsubseteq a_k$. If $\mu_I = \top$ then $\{a_i\}_{i\in I}=\{\top\}$,
    so we have $a_k=\top$, hence $\mu_I \sqsubseteq a_k$.
    If $\mu_I=\bot$ then $\mu_I\sqsubseteq a_k$.
    Otherwise (\textit{ie.} $\mu_I\neq\top,\bot$ and
    $a_k\neq\top,\bot$), let $x\to^m t\in \mu_I$. Then, for all $i\in I$ either
    ...
    If % \issue{$\mu'_I$ is empty or}{I would remove this}
    $\mu_I=\top$ then $\mu'_I \sqsubseteq \mu_I$.
    If $\mu'_I=\top$ then necessarily $\{a_i\}_{i\in I}=\{\top\}$, so
    we have $\mu_I=\top$, hence $\mu'_I \sqsubseteq \mu_I$.
    Otherwise (\textit{ie.} % \issue{$\mu'_I\neq\emptyset$,}{I would remove this}
    $\mu'_I\neq\top$ and $\mu_I\neq\top$),
...
\begin{lemma}\label{lem:co-additive-type}
  Let $I\subseteq\mathbb{N}$ and $\{t_i\}_{i\in I}\subseteq\mathsf{T}_\kappa$ be such that
  $\sqcap_{i\in I}t_i$ is defined.
  For any $\rho\star h\in\mathbb{S}_\kappa$ and $\ell\in\Locs$,
  if $\ell\approx_{\rho\star h}\sqcap_{i\in I}t_i$ then
  $\ell\approx_{\rho\star h}t_i$ for all $i\in I$.
\end{lemma}
\begin{proof}
  ...
  \begin{itemize}
    ...
    Suppose that $\ell\approx_{\rho\star h}\sqcap_{i\in I}t_i$.
    If $t_k=\bot$ then $\sqcap_{i\in I}t_i = \bot$, hence
    $h(\ell)$ is undefined. So,
    \issue{$\ell\approx_{\rho\star h}t_k$}{What about the other $t_i$'s?}.
    If $t_k=\mathsf{int}$ then either $\sqcap_{i\in I}t_i = \bot$ (this happens
    when there is a $t_{k'}$ which is not $\mathsf{int}$) or
    $\sqcap_{i\in I}t_i = \mathsf{int}$ (this happens when each $t_i$ is
    $\mathsf{int}$);
    in the former case, \issue{$h(\ell)$ is undefined}{so we do not have $\ell\approx_{\rho\star h}t_k$};
    in the latter case, we have $h(\ell)\in\mathbb{Z}$
    so $\ell\approx_{\rho\star h}t_i$ for all $i\in I$.
    %
    \item (Induction step) Suppose that $t_i=\mathtt{box}\ t'_i$ for all
    $i\in I$. Suppose also that for any
    $\rho\star h\in\mathbb{S}_\kappa$ and $\ell\in\Locs$, if
    $\ell\approx_{\rho\star h}\sqcap_{i\in I}t'_i$ then
    $\ell\approx_{\rho\star h}t'_i$ for all $i\in I$.
    Let $\rho\star h\in\mathbb{S}_\kappa$ and $\ell\in\Locs$.
    If $\ell\approx_{\rho\star h}\sqcap_{i\in I}t_i$ then
    $\ell\approx_{\rho\star h} \mathtt{box}\ \sqcap_{i\in I}t'_i$. So,
    $\ell\Rightarrow\ell'\in h$ and $\ell'\approx_{\rho\star h}\sqcap_{i\in I}t'_i$.
    Hence, by induction hypothesis, $\ell'\approx_{\rho\star h}t'_i$ for all $i\in I$.
    Consequently, for all $i\in I$, $\ell\approx_{\rho\star h} \mathtt{box}\ t'_i$
    \ie $\ell\approx_{\rho\star h} t_i$.
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $\gamma_\kappa(\sqcap_{i\in I}a_i)\subseteq\cap_{i\in I}\gamma_\kappa(a_i)$.
    Let $\rho\star h\in\gamma_\kappa(\sqcap_{i\in I}a_i)$.
    Then for every $x\to^m\ell\in\rho$ there is $x\to^m t\in \sqcap_{i\in I}a_i$ s.t.
    $\ell\approx_{\rho\star h}t$. By definition of $\sqcap$, for all $i\in I$
    either $a_i=\top$ or there is $x\to^m t_i\in a_i$, and
    $t=\sqcap_{i\in J}t_i$ where $J=\{i\in I\mid a_i\neq\top\}$.
    \item Now we prove that
    $\gamma_\kappa(\sqcap_{i\in I}a_i)\supseteq\cap_{i\in I}\gamma_\kappa(a_i)$.
  \end{itemize}
  \qed
\end{proof}
