...
\begin{proof}
  % TODO
  Suppose that $\mathsf{w}$ is undefined in $s$. Then there are two possibilities.
  \begin{itemize}
    \item Either $\loc(\mathsf{w},s)$ is undefined in $s$.
    Then, if $\mathsf{w}\in\Vars$, we have $\mathsf{w}\not\in\dom(\kappa)$
    and so $a(\mathsf{w})$ is undefined which implies that $\loca(\mathsf{w},a)$
    is undefined \ie that $\mathsf{w}$ is undefined in $a$.
    If $\mathsf{w}$ has the form $\mathtt{*}\mathsf{w}'$ for some leftvalue $\mathsf{w}'$,
    \item Or $\loc(\mathsf{w},s)$ is dangling in $s$.
  \end{itemize}
\end{proof}
