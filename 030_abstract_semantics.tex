...
\begin{proof}
  ...
  \item Assume that the result holds for $n\ge 0$ and consider
    ...
    $a(\mathsf{root}(\mathtt{*}\mathsf{w}))=a(r)=\underbrace{\boxempty\cdots\boxempty}_\text{$n$}\boxtype{t}
    =\underbrace{\boxempty\cdots\boxempty}_\text{$n+1$}t$.
  \end{itemize}
  \qed
\end{proof}
...
\begin{proof}
  ...
  $a(\mathsf{root}(\mathsf{w}_i'))=\underbrace{\boxempty\cdots\boxempty}_\text{$m_i$}t_i$,
  where $\mathsf{w}_i'=\underbrace{\mathtt{*}\cdots\mathtt{*}}_{\text{$m_i$}}\mathsf{root}(\mathsf{w}_i')$.
