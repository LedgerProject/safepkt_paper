...
\begin{proof}
  ...
  \begin{itemize}
    \item (Reflexivity) Let $a\in\mathbb{AS}_\kappa$. If $a=\top$, then
      $a\sqsubseteq a$.
      % \issue{Otherwise, if $a$ is empty, then $a\sqsubseteq a$ holds
      % vacuously.}{I would remove this}
...
\begin{definition}[Concretization map]\label{def:concretization}
  ...
  \begin{align*}
    ...
    \ell&\approx_{\rho\star h}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} & \text{iff $\exists i.1\le i\le n$ s.t.\ }\ell\to\loc(\mathsf{w}_i,s)\in\rho\\
    \ell&\approx_{\rho\star h}\mathtt{mut}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} & \text{iff $\exists i.1\le i\le n$ s.t.\ }\ell\leadsto\loc(\mathsf{w}_i,s)\in\rho\\
