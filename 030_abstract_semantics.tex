...
\begin{definition}[Value compatibility]\label{def:value_compatibility}
  Let $v\in\mathbb{V}$ be a value, $\rho\star h\in\mathbb{S}_\kappa$, $\mathsf{w}\in\Leftvalues_\kappa$
  and $t\in\mathsf{T}_\kappa$ not a box. Then $v$ is \emph{compatible} with $t$ in $\rho\star h$,
  written $v\sim_{\rho\star h}^\mathsf{w}t$, is defined as
  \begin{align*}
    v&\sim_{\rho\star h}^\mathsf{w}\mathsf{int} && \text{iff $v\in\mathbb{Z}$}\\
    v&\sim_{\rho\star h}^\mathsf{w}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} && \text{iff $v\in\Locs$ and}\\
    &&& \exists i\in\{1,\ldots,n\}.\loc(\mathsf{w}_i,\rho\star h)=v\text{ and}\\
    &&& \forall\mathsf{w'}\in\Leftvalues.\left(\mathsf{w}\not=\mathsf{w}'\text{ and }\loc(\mathsf{w}',\rho\star h)=v\text{ implies }\mathsf{w}'\in\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}\right)\\
    v&\sim_{\rho\star h}^\mathsf{w}\mutborrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} && \text{iff $v\in\Locs$ and}\\
    &&& \exists i\in\{1,\ldots,n\}.\loc(\mathsf{w}_i,\rho\star h)=v\text{ and}\\
    &&& \forall\mathsf{w}\in\Leftvalues.\left(\mathsf{w}\not=\mathsf{w}'\text{ and }\loc(\mathsf{w}',\rho\star h)=v\text{ implies }\mathsf{w}'\in\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}\right)\\
    v&\sim_{\rho\star h}^\mathsf{w}\mathsf{dangling} && \text{never.}
  \end{align*}
\end{definition}
...
\begin{definition}[Concretization map]\label{def:concretization}
  ...
  \text{for every }x\to^m\ell\in\rho\text{ there is }x\to^m t\in a\text{ s.t.\ }\ell\approx_{\rho\star h}^x t
  \end{array}
  \right.\right\}
  \]
  where $\ell\approx_{\rho\star h}^\mathsf{w}t$ is defined as
  \begin{align*}
    \ell&\approx_{\rho\star h}^\mathsf{w}\mathsf{int} && \text{iff $h(\ell)\sim_{\rho\star h}\mathsf{int}$}\\
    \ell&\approx_{\rho\star h}^\mathsf{w}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} && \text{iff $\exists\ell'.\ell\to\ell'\in h$ and $\ell'\sim_{\rho\star h}^{\mathtt{*}\mathsf{w}}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$}\\
    \ell&\approx_{\rho\star h}^\mathsf{w}\mutborrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} && \text{iff $\exists\ell'.\ell\leadsto\ell'\in h$ and $\ell'\sim_{\rho\star h}^{\mathtt{*}\mathsf{w}}\mutborrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$}\\
    \ell&\approx_{\rho\star h}^\mathsf{w}\boxtype{t} && \text{iff $\exists\ell'.\ell\Rightarrow\ell'\in h$ and $\ell'\approx_{\rho\star h}^{\mathtt{*}\mathsf{w}}t$}\\
    \ell&\approx_{\rho\star h}^\mathsf{w}\mathsf{dangling} && \text{iff $h(\ell)$ is undefined.}
  \end{align*}
\end{definition}

The role of the parameter $\mathsf{w}$ in Def.~\ref{def:value_compatibility}
and~\ref{def:concretization}
is to avoid the use of identities in borrows. Without that $\mathsf{w}$
and its consequent use to define the compatibility of a value with a borrow,
the abstract domain element
\[
  \{x\to^m\borrow\{y\}, y\to^m\mathsf{int}\}
\]
would have an empty concretization, since the location of $\mathtt{*}x$ coincides
with the value of $x$, always. Hence we would be forced to add $\mathtt{*}x$ to the borrow:
\[
  \{x\to^m\borrow\{\mathtt{*}x, y\}, y\to^m\mathsf{int}\}.
\]
But that $\mathtt{*}x$ is tautological information. Moreover, it makes the dependencies
of the typing cyclical (Def.~\ref{def:dependencies}) since it entails that
$\mathtt{*}x\gg\mathtt{*}x$. In order to avoid this problem, we do not require the
presence of such tautological information, by restricting the quantifications in
Def.~\ref{def:value_compatibility}.
...
\begin{proposition}[Abstract locations are correct]
  ...
  \begin{enumerate}
  \item $\ell'\approx_s^\mathsf{w}t$
  \item $p=\mathit{true}$ implies $p'=\mathit{true}$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  Let $\rho\star h=s$. We proceed by induction on $\mathsf{w}$ wrt.\ the $\gg$ relation.
  \begin{itemize}
  \item (Base: there is no $\mathsf{w}'$ such that $\mathsf{w}\gg\mathsf{w}'$)
    By Def.~\ref{def:dependencies}, $\mathsf{w}$ is a variable.
    ...
    $\mathsf{w}\to^m t\in a$ s.t. $\ell'\approx_s^\mathsf{w} t$.
    Moreover, by Def.~\ref{def:locreada},
    $\locreada(\mathsf{w},a)=\langle a(\mathsf{w}),\mathit{false}\rangle
    =\langle t,\mathit{false}\rangle$.
    %
  \item (Induction step: there are $\mathsf{w}'$ such that $\mathsf{w}\gg\mathsf{w}'$ and the result
    holds for them; we prove it for $\mathsf{w}$)
    By Def.~\ref{def:dependencies}, $\mathsf{w}=\mathtt{*}\mathsf{w''}$ for some $\mathsf{w}''$.
    As $\locread(\mathsf{w},s)$ is defined,
    by Def.~\ref{def:location_for_reading} there are three possibilities.
    \begin{itemize}
      \item Either $\locread(\mathsf{w},s)=\langle\ell',\mathit{true}\rangle$,
      $\locread(\mathsf{w}'',s) = \langle \ell,b\rangle$ and $\ell\to\ell'\in h$.
      Condition (2) holds in this case.
      By Prop.~\ref{prop:acyclicity} we can use the induction hypothesis on $\mathsf{w}''$
      and $\locreada(\mathsf{w}'',a)=\langle t',b'\rangle$ with $\ell\approx_s^{\mathsf{w}''}t'$,
      ...
      and $\ell'\sim_s^{\mathtt{*}\mathsf{w}''}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$, that is,
      $\ell'\sim_s^{\mathsf{w}}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$.
      By Def.~\ref{def:value_compatibility}, it exists $i\in\{1,\ldots,n\}$
      such that $\loc(\mathsf{w}_i,s)=\langle\ell',b_i\rangle$.
      By Prop.~\ref{prop:acyclicity}, we can apply the inductive hypothesis to $\mathsf{w}_i$
      and conclude that $\locreada(\mathsf{w}_i,a)=\langle t'',b_i'\rangle$ with
      $\ell'\approx_s^{\mathsf{w}_i}t''$ and $b_i$ true implies $b_i'$ true.
      By Def.~\ref{def:locreada}, we have $t''\sqsubseteq t$. Hence
      $\ell'\approx_s^{\mathsf{w}_i}t$.
