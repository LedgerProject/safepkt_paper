...
\begin{proof}
  ...
  \item (Induction step) Suppose that the thesis holds for $\mathsf{w}$. Let us prove it
    ...
    \begin{itemize}
      ...
      $a(\mathsf{root}(\mathsf{w}_i))=\underbrace{\boxempty\cdots\boxempty}_{\text{$m$ times}}t''$, with
      $\mathsf{w}_i=\underbrace{\mathtt{*}\cdots\mathtt{*}}_{\text{$m$ times}}\mathsf{root}(\mathsf{w}_i)$.
      Since $s\in\gamma_\kappa(a)$, it follows that
      $s(\mathsf{root}(\mathsf{w}_i))\approx_s a(\mathsf{root}(\mathsf{w}_i))
      =\underbrace{\boxempty\cdots\boxempty}_{\text{$m$ times}}t''$ and therefore
