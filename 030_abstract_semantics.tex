...
\begin{definition}[Concretization map]\label{def:concretization}
  ...
  \begin{align*}
    ...
    &&& \text{$\forall\mathsf{w}\in\Leftvalues.\loc(\mathsf{w},\rho\star h)=\ell'$ implies $\mathsf{w}\in\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$}\\
    \ell&\approx_{\rho\star h}\mathtt{mut}\borrow\{\mathsf{w}_1,\ldots,\mathsf{w}_n\} && \text{iff $\exists\ell'.\ell\leadsto\ell'\in h$ and}\\
    &&& \text{$\forall\mathsf{w}\in\Leftvalues.\loc(\mathsf{w},\rho\star h)=\ell'$ implies $\mathsf{w}\in\{\mathsf{w}_1,\ldots,\mathsf{w}_n\}$}\\
