...
\begin{definition}[Leftvalue unwinding]\label{def:unwind}
...
\begin{definition}[Abstract stores]\label{def:abstract-store}
  ...
  \item \textbf{Type consistency:} the type
    \[
    t(\mathsf{w}_1,\ldots,\mathsf{w}_n,a)=\sqcup\{t\mid\locreada(\mathsf{w}_i,a)=\langle t,\_\rangle
    \text{ and }\unwind(\mathsf{w}_i,a)\cap\Leftvalues=\varnothing\}
    \]
    is defined, where, for $\mathsf{w}\in\Leftvalues$, the partial function
...
\begin{lemma}\label{lem:unwind_no_leftvalues}
  Given a context $\kappa$, an abstract store $a\not\in\{\top,\bot\}$ over $\kappa$ and
  $\mathsf{w}\in\Leftvalues_\kappa$ such that
  $\locreada(\mathsf{w},a)$ is defined and $\unwind(\mathsf{w},a)\cap\Leftvalues=\varnothing$,
  then $a(\mathsf{root}(\mathsf{w}))=\underbrace{\boxempty\cdots\boxempty}_\text{$n$ types}t$ for some $t$,
...
\begin{proof}
  ...
  \item assume that the result holds for $n\ge 0$ and consider
    ...
    By Def.~\ref{def:unwind}, since $\unwind(\mathtt{*}\mathsf{w},a)\cap\Leftvalues=\varnothing$ it follows that
    $\unwind(\mathsf{w},a)\cap\Leftvalues=\varnothing$. By inductive hypothesis, it is
    $a(r)=\underbrace{\boxempty\cdots\boxempty}_\text{$n$ types}t'$ for some $t'$.
    By Def.~\ref{def:abstract-store}, it is $\locreada(\mathsf{w},a)=\langle t',\_\rangle$.
    ...
    either $\unwind(\mathtt{*}\mathsf{w},a)=\varnothing$ or
    $t\in\unwind(\mathtt{*}\mathsf{w},a)$ with $t'=\boxtype{t}$. In the latter case, it is
    $a(r)=\underbrace{\boxempty\cdots\boxempty}_\text{$n$ types}\boxtype{t}
    =\underbrace{\boxempty\cdots\boxempty}_\text{$n+1$ types}t$ and the thesis follows.
    In the former case, we have excluded that $t'$ is a box, but it cannot either be a borrow
    nor a mutable borrow, since by Def.~\ref{def:unwind} the set
    $\unwind(\mathtt{*}\mathsf{w},a)\cap\Leftvalues$ could not be empty then. Hence $t'$ can only be $\mathsf{int}$
    or $\mathsf{dangling}$, that is, $\locreada(\mathsf{w},a)=\langle\mathtt{int},\_\rangle$ or
    $\locreada(\mathsf{w},a)=\langle\mathtt{dangling},\_\rangle$. Therefore, by Def.~\ref{def:abstract-store},
    it follows that $\locreada(\mathtt{*}\mathsf{w},a)$ is undefined, which is against our hypotheses.
    We conclude that the former case cannot occur.
  \end{itemize}
  \qed
\end{proof}
...
\begin{proof}
  ...
  \item (Induction step) Suppose that the thesis holds for $\mathsf{w}$. Let us prove it
    ...
    \begin{itemize}
      ...
      By definition of $\locreada(\mathsf{w},a)$, we have that $t'$ occurs in a binding in $a$.
      Hence, by Def.~\ref{def:concretization}, there exists $t$ such that
      for all $1\le i\le n$ we have $\locreada(\mathsf{w}_i,a)=\langle t,\_\rangle$ and
      $\locreada(\mathtt{*}\mathsf{w},a)=\langle t,\mathit{true}\rangle$.
      It follows that the second point of the thesis holds.
      We must prove that $\ell'\approx_st$.
